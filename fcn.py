...
class FCN16s(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.relu(self.deconv1(x5))               # size=(N, 512, x.H/16, x.W/16)
        x = self.bn1(x + x4)                      # element-wise add, size=(N, 512, x.H/16, x.W/16)
        x = self.bn2(self.relu(self.deconv2(x)))  # size=(N, 256, x.H/8, x.W/8)
        x = self.bn3(self.relu(self.deconv3(x)))  # size=(N, 128, x.H/4, x.W/4)
        x = self.bn4(self.relu(self.deconv4(x)))  # size=(N, 64, x.H/2, x.W/2)
        x = self.bn5(self.relu(self.deconv5(x)))  # size=(N, 32, x.H, x.W)
...
class FCN8s(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.bn2(x + x3)                      # element-wise add, size=(N, 256, x.H/8, x.W/8)
        x = self.bn3(self.relu(self.deconv3(x)))  # size=(N, 128, x.H/4, x.W/4)
        x = self.bn4(self.relu(self.deconv4(x)))  # size=(N, 64, x.H/2, x.W/2)
        x = self.bn5(self.relu(self.deconv5(x)))  # size=(N, 32, x.H, x.W)
        x = self.classifier(x)                    # size=(N, n_class, x.H/1, x.W/1)
...
class FCNs(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.bn1(self.relu(self.deconv1(x5)))     # size=(N, 512, x.H/16, x.W/16)
        x = x + x4                                # element-wise add, size=(N, 512, x.H/16, x.W/16)
        x = self.bn2(self.relu(self.deconv2(x)))  # size=(N, 256, x.H/8, x.W/8)
        x = x + x3                                # element-wise add, size=(N, 256, x.H/8, x.W/8)
        x = self.bn3(self.relu(self.deconv3(x)))  # size=(N, 128, x.H/4, x.W/4)
        x = x + x2                                # element-wise add, size=(N, 128, x.H/4, x.W/4)
        x = self.bn4(self.relu(self.deconv4(x)))  # size=(N, 64, x.H/2, x.W/2)
        x = x + x1                                # element-wise add, size=(N, 64, x.H/2, x.W/2)
        x = self.bn5(self.relu(self.deconv5(x)))  # size=(N, 32, x.H, x.W)
        x = self.classifier(x)                    # size=(N, n_class, x.H/1, x.W/1)
...
if __name__ == "__main__":
    ...
    inputs = torch.autograd.Variable(torch.randn(batch_size, 3, h, w))
    # fcn_model = fcn_model.cuda()
    # inputs = inputs.cuda()
    # print(summary(fcn_model, (3, h, w)))
